...
export default class ListItem extends Component {
  ...
  render() {
    ...
    const lastVisited = this.props.lastVisited ? this.props.lastVisited : 'no-last-visits';

    return (
      <li className="link-history-item">
        <div className="link">
          <div className="link-shorten url-body">
            <CopyToClipboard text={'localhost:8080/api/v1/' + shortLinkIdentifier}>
              <div id="short-link"
                onClick={() => console.log('short link clicked')}>
                  localhost:8080/api/v1/<span style={{color:'#EB4A42'}}>{shortLinkIdentifier}</span>
              </div>
            </CopyToClipboard>
            <div id="copy-message" >
