...
export default class UrlList extends Component {
  ...
  componentWillReceiveProps(nextProps) {
        ...
        for (let i = 0; i < nextProps.shortenedLinkList.length; i++) {
            ...
            tmpLinkList.push(
                <ListItem key={tmpLinkItem.keycode}
                          shortLinkIdentifier={tmpLinkItem.keyCode}
                          originalLink={tmpLinkItem.longUrl}
                          visits={tmpLinkItem.totalVisits}
                          lastVisited={tmpLinkItem.lastAccessDate}
                />
            )
        }
